...
/* USER CODE BEGIN PFP */
uint8_t init_alloc_heap();
volatile void soft_Delay(uint32_t time_i);
/* USER CODE END PFP */
...
uint8_t init_alloc_heap()
{
		...
		#define HEAP_SIZE 0x00000010
		#define HEAP_END  HEAP_BASE+HEAP_SIZE
		_init_alloc(HEAP_BASE,HEAP_END);
}

volatile void soft_Delay(uint32_t time_ms)
{
	uint16_t i = 0;
	while(time_ms--)
	{
